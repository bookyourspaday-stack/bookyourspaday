...
import { Calendar, Mail, Phone, ShoppingBag, Users, Video } from "lucide-react";
...
const paymentLinks = {
  ...
  paypal: import.meta.env.VITE_PAYPAL_LINK || "",
};

const services = [["Swedish Massage", "Light\u2013medium pressure, full\u2011body relaxation", "/img/IMG_0002.jpeg"], ["Deep Tissue Massage", "Targeted pressure for chronic tension", "/img/IMG_0002.jpeg"], ["Sports Massage", "Recovery, mobility, trigger point work", "/img/IMG_6033.jpeg"], ["Chair Massage", "On\u2011site stress relief for events & workplaces", "/img/D7CD3463-EFE1-4123-81A1-60EEFEF6293A.JPG"], ["Lymphatic Drainage", "Gentle detox support, post\u2011op friendly", "/img/service-lymph.jpg"], ["Hot Stone Massage", "Warm basalt stones, deep relaxation", "/img/service-hotstone.jpg"]];
const team = [["Maria Cristina Martinez", "Deep Tissue \u2022 Sports \u2022 Wellness", "/img/IMG_1741.jpg"], ["Ava", "Deep Tissue & Trigger Point", "/img/service-prenatal.jpg"], ["Noah", "Sports & Stretch Therapy", "/img/service-hotstone.jpg"], ["Maya", "Lymphatic & Post\u2011Op", "/img/service-lymph.jpg"]];
const experienceVideos = ["/media/experience-1.mp4", "/media/experience-2.mp4", "/media/experience-3.mp4", "/media/experience-4.mp4", "/media/experience-5.mp4", "/media/experience-6.mp4", "/media/experience-7.mp4", "/media/experience-8.mp4"];
    ...
    <div className="min-h-screen bg-base text-white">
      <header className="sticky top-0 z-40 backdrop-blur bg-black/20 border-b border-white/10">
        <div className="container flex items-center justify-between py-3">
          ...
          <nav className="hidden md:flex gap-6 text-sm opacity-90">
            ...
            {experienceVideos.length ? <a href="#experience">Experience</a> : null}
            <a href="#ayurveda">Ayurveda</a>
            <a href="#store">Store</a>
            <a href="#team">Therapists</a>
      ...
      <section id="booking" className="section">
        <div className="container grid md:grid-cols-2 gap-8">
          ...
          <form onSubmit={(e)=>{e.preventDefault(); window.location.href=mailto;}} className="card p-6 grid gap-3">
            <input className="bg-transparent border border-white/15 rounded-2xl px-4 py-3" placeholder="Name" value={form.name} onChange={e=>setForm({...form, name:e.target.value})} required/>
            <input className="bg-transparent border border-white/15 rounded-2xl px-4 py-3" placeholder="Email" type="email" value={form.email} onChange={e=>setForm({...form, email:e.target.value})} required/>
            <input className="bg-transparent border border-white/15 rounded-2xl px-4 py-3" placeholder="Phone" value={form.phone} onChange={e=>setForm({...form, phone:e.target.value})}/>
            <textarea className="bg-transparent border border-white/15 rounded-2xl px-4 py-3 h-28" placeholder="Treatments / notes" value={form.notes} onChange={e=>setForm({...form, notes:e.target.value})}/>
      ...
      <section id="services" className="section">
        <div className="container">
          ...
          <p className="mt-2 text-white/80">Real photos + examples—every session is customized.</p>
          <div className="mt-6 grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {services.map(([title, subtitle, src], i)=>(
      ...
      {experienceVideos.length ? (
      <section id="experience" className="section">
        <div className="container">
          <h2 className="text-2xl font-semibold flex items-center gap-2"><Video className="w-5 h-5" /> Experience</h2>
          <p className="mt-2 text-white/80">Short clips from real sessions and events.</p>
          <div className="mt-6 grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {experienceVideos.map((v,i)=>(
              <video key={i} src={v} playsInline autoPlay muted loop controls className="w-full aspect-video object-cover" />
            ))}
          </div>
        </div>
      </section>) : null}
        ...
        <div className="container grid md:grid-cols-2 gap-8 items-start">
          ...
          <div className="card p-6">
            ...
            <div className="mt-5 grid sm:grid-cols-3 gap-3">
              {paymentLinks.stripe ? <a href={paymentLinks.stripe} target="_blank" rel="noreferrer" className="btn btn-primary">Pay with Stripe</a> : <a href={orderMail} className="btn btn-primary">Buy via Email</a>}
              {paymentLinks.paypal ? <a href={paymentLinks.paypal} target="_blank" rel="noreferrer" className="btn">Pay with PayPal</a> : <a href="#payment" className="btn">Pay with Zelle</a>}
              <a href="#payment" className="btn">Zelle Details</a>
            </div>
      ...
      <section id="team" className="section">
        <div className="container">
          <h2 className="text-2xl font-semibold"><Users className="inline w-5 h-5 mr-2" /> Our Therapists</h2>
          <div className="mt-6 grid sm:grid-cols-2 md:grid-cols-4 gap-4">
            {team.map(([name, spec, src], i)=>(
              <div key={i} className="card overflow-hidden">
                <img src={src} alt={name} className="w-full aspect-[3/4] object-cover" />
                <div className="p-4">
                  <div className="font-medium">{name}</div>
                  <div className="text-sm text-white/70">{spec}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="payment" className="section">
        <div className="container grid md:grid-cols-3 gap-6">
          ...
          <div className="card p-5">
            ...
            <div className="mt-1 text-white/80">Request an invoice via email or use the button above if configured.</div>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="section">
        <div className="container grid md:grid-cols-2 gap-8">
          ...
          <div className="card p-6 text-sm text-white/70">
            *Font: Site uses <strong>Alta</strong> when you paste your Adobe kit in <code>index.html</code>.
          </div>
        </div>
      </section>
